...
The benchmarks above where obtained using <code>export MKL_NUM_THREADS=1</code> and <code>export OMP_NUM_THREADS=1</code> increasing the <code>MKL_NUM_THREADS</code> may improve the results for both the MKL and the MAGMA versions. Furthermore, unlike the benchmarks shown in MAGMA testing implementations these benchmarks account for the memory transfer times from Host to Device. This is the reason why the dgemv and dtrsm do not seem to perform better than the CPU versions. 
